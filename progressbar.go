...
func OptionSetBytes(maxBytes int64) Option {
	return func(p *ProgressBar) {
...
func NewOptions(max int64, options ...Option) *ProgressBar {
	b := ProgressBar{
		state: getBlankState(),
		config: config{
			writer:           os.Stdout,
...
func New(max int64) *ProgressBar {
...
func (p *ProgressBar) Set(num int64) error {
	p.lock.Lock()
	defer p.lock.Unlock()
